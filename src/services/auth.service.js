...
import { collection, getDocs, query, where, deleteDoc, doc, updateDoc, addDoc, orderBy, Timestamp, } from "firebase/firestore";
...
export const getTanamanData = async (jenis, musim, searchQuery) => {
    try {
        // Build the Firestore query with optional filters
        const q = query(collection(db, "jadwal_tanam"), ...(jenis ? [where("jenis_tanaman", "==", jenis)] : []), ...(musim ? [where("musim", "==", musim)] : []), ...(searchQuery
            ? [
...
export const deleteJadwalTanam = async (id) => {
    try {
...
export const updateJadwalTanam = async (id, updatedData) => {
    try {
...
export const createJadwalTanam = async (newJadwal) => {
    try {
    ...
    try {
        ...
        const comments = querySnapshot.docs.map((doc) => {
            ...
            return {
                ...
                createAt,
                postId: data.postId || "",
            }; // Explicit cast to your expected Comment type
        });
        return comments;
    }
    catch (error) {
...
// };
export { getUserCount, getPostCount, getTutorialCount, getPlantingScheduleCount, getUsers, deleteUser, };
